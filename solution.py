...
for i in range(1, 4):
    print("bar", number_to_letter(i), end=" ")
...
def getAdjNode(node):
    ...
    for dir in DIRECTIONS.keys():
        ...
        if 0 <= newPos[0] < len(node.current_node) and 0 <= newPos[1] < len(
            node.current_node[0]
        ):
...
def main(puzzle):
    ...
    while True:
        ...
        for node in adj_node:
            if (
                str(node.current_node) in closed_set.keys()
                or str(node.current_node) in open_set.keys()
                and open_set[str(node.current_node)].f() < node.f()
            ):
                continue
...
def solve_robo(array) :
    ...
    for b in br:
        if b["dir"] != "":
            ...
            if b["dir"] == "U":
                letter = "UP"
                Ans_list.append( str(i) +'up')
            elif b["dir"] == "R":
                letter = "RIGHT"
                Ans_list.append(str(i) +'right')
            elif b["dir"] == "L":
                letter = "LEFT"
                Ans_list.append(str(i) +'left')
            elif b["dir"] == "D":
                letter = "DOWN"
                Ans_list.append(str(i) +'down')
    ...
    print(
        dash + dash + right_junction, "ABOVE IS THE OUTPUT", left_junction + dash + dash
    )
    ...
    if( len(Ans_list)) ==0 :
        ...
        #         array.index(i).index(j) == END.index(i).index(i)
